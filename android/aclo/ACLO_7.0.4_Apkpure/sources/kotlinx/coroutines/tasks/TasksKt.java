...
public final class TasksKt {
    ...
    private static final <T> Deferred<T> asDeferredImpl(Task<T> task, final CancellationTokenSource cancellationTokenSource) {
        ...
        if (task.isComplete()) {
            ...
            task.addOnCompleteListener(DirectExecutor.INSTANCE, new OnCompleteListener() { // from class: kotlinx.coroutines.tasks.-$$Lambda$TasksKt$L3ThvVNcb-_HszwYR6dxmg9BneU
                @Override // com.google.android.gms.tasks.OnCompleteListener
                public final void onComplete(Task task2) {
                    TasksKt.m1562asDeferredImpl$lambda0(CompletableDeferred.this, task2);
                }
            });
        }
    ...
    public static final void m1562asDeferredImpl$lambda0(CompletableDeferred completableDeferred, Task task) {
        Exception exception = task.getException();
        if (exception == null) {
            if (task.isCanceled()) {
        ...
        task.addOnCompleteListener(DirectExecutor.INSTANCE, new OnCompleteListener() { // from class: kotlinx.coroutines.tasks.TasksKt$awaitImpl$2$1
            ...
            public final void onComplete(Task<T> task2) {
                ...
                if (exception2 == null) {
                    ...
                    cancellableContinuationImpl2.resumeWith(Result.m12constructorimpl(task2.getResult()));
                    return;
                }
                Result.Companion companion2 = Result.Companion;
                cancellableContinuationImpl2.resumeWith(Result.m12constructorimpl(ResultKt.createFailure(exception2)));
            }
        });
        if (cancellationTokenSource != null) {
